...
# --- Open MPI over UCX, no fancy collectives ---
...
# Run the MPI program
mpirun --rankfile rankfile.txt \
  --report-bindings \
  --mca coll ^ucc,hcoll,sharp \
  --mca coll_tuned_use_dynamic_rules 1 \
  --mca coll_tuned_bcast_algorithm 4 \
  --mca coll_tuned_bcast_segment_size 65536 \
  -x PATH -x LD_LIBRARY_PATH \
  -x AOCLROOT -x OMP_NUM_THREADS -x OMP_PROC_BIND -x OMP_PLACES \
  -x BLIS_ENABLE_OPENMP -x BLIS_NUM_THREADS -x BLIS_CPU_EXT -x BLIS_DYNAMIC_SCHED \
