...
# UCX transports: shared memory + CUDA IPC paths only
export UCX_TLS=sm,self,cuda_ipc,cuda_copy,gdr_copy
export UCX_SHM_DEVICES=posix,sysv
export UCX_MEMTYPE_CACHE=n          # optional: avoid caching race issues
export UCX_NET_DEVICES=all          # harmless default

# Explicitly disable the problematic/unused modules
export UCX_CMA_ENABLE=n
export UCX_TLS_SHARED_DISABLE=cma,knem
