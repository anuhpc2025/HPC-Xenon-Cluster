...
mpirun \
  ...
  -x UCX_MEMTYPE_CACHE \
  -x UCX_RNDV_SCHEME \
  -x UCX_IB_PCI_RELAXED_ORDERING \
  -x LD_LIBRARY_PATH \
  -x PATH \
  bash -c 'export CUDA_VISIBLE_DEVICES=$SLURM_LOCALID; ./xhpl_mxp-nvidia \
  --nprow 2 --npcol 2 --nporder row --sloppy-type 2 --n 120000 --nb 384 --preset-gemm-kernel 0'