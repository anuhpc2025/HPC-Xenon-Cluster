...
#SBATCH --ntasks-per-node=1             # 1 MPI rank per node (2 ranks total)
#SBATCH --cpus-per-task=2               # 8 threads per rank
#SBATCH --time=12:00:00                 # Time limit hh:mm:ss
#SBATCH --nodelist=node1,node3          # Explicit node list
...
echo "N nodes   : ${SLURM_JOB_NUM_NODES}"
...
########################################
# 5. Config graph dump (single-rank SST)
########################################
...
# Single-rank SST just to dump the configuration graph (no mpirun here)
sst \
...
# Show first few lines of the DOT file in the Slurm/log, if it exists
if [ -f "${GRAPH_DOT}" ]; then
  echo "==== First lines of ${GRAPH_DOT} ====" | tee -a "${LOG_FILE}"
  head -n 40 "${GRAPH_DOT}" | tee -a "${LOG_FILE}"
  echo "=====================================" | tee -a "${LOG_FILE}"
else
  echo "WARNING: DOT file not created: ${GRAPH_DOT}" | tee -a "${LOG_FILE}"
fi

########################################
# 6. Parallel SST run with mpirun across 2 nodes
########################################
