...
# Element search path (what sst uses to find libfirefly.so, libmerlin.so, etc.)
export SST_ELEMENT_SEARCHPATH="${SST_CORE_LIBDIR}:${SST_ELEM_LIBDIR}:/home/hpc/Downloads/sst-build/sst-elements/src/sst/elements/osseous"
...
mpirun \
  ...
  sst \
    ...
    --add-element-lib=merlin \
    --add-element-lib=firefly \
    --num-threads="${threads}" \
