...
# --- Load HPC-X + UCX like in the HPL script ---
# (optional) bring in your usual shell env
source ~/.bashrc

# HPC-X root
export HPCX_HOME=/home/hpc/hpcx/hpcx-v2.24-gcc-doca_ofed-ubuntu24.04-cuda13-x86_64

# Ensure UCX + OMPI binaries/libs are ahead in PATH / LD_LIBRARY_PATH
...
# Initialize HPC-X environment and load UCX + OMPI stack
if [ -f "${HPCX_HOME}/hpcx-init.sh" ]; then
    source "${HPCX_HOME}/hpcx-init.sh"
    # This loads the UCX and OMPI components from HPC-X
    hpcx_load ucx ompi
fi
sync
########################################
...
j=10                 # jobs: number of jobs per group
k=5                 # iterations: number of halo exchanges (higher k => longer simulation)
