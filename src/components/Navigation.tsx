...
interface NavigationProps {
  ...
  onSuiteChange: (suite: BenchmarkSuite) => void;
}
...
const getIcon = (type: string) => {
  switch (type) {
    ...
    case 'GPU': return <Zap className="w-5 h-5" />;
    case 'Climate': return <Cloud className="w-5 h-5" />;
    case 'Structural': return <Building className="w-5 h-5" />;
    default: return <Cpu className="w-5 h-5" />;
  }
};

export const Navigation: React.FC<NavigationProps> = ({ activeSuite, onSuiteChange }) => {
  return (
